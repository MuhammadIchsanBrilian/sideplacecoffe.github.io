...
$brand = mysqli_query($koneksi, "SELECT * FROM brand WHERE id='$id'");
$result = mysqli_fetch_all($brand, MYSQLI_ASSOC);
?>
<!DOCTYPE html>
...
<head>
	...
	<meta charset="UTF-8">
	<!-- Site Title -->
	<title>SIDEPLACE - ADMIN FEATURES</title>
		...
		<div class="container" style="background:white; width: 420px; border-radius:10px;">
			<div class="row d-flex justify-content-center">
				<div class="menu-content pb-60 col-lg-10">
					<div class="title text-center mt-4">
						<h1 class="mb-10" style="color:black;">Edit Brand</h1>
					</div>
					<form method="POST" action="edit_brand_act.php">
					
                    <?php
            if (isset($_GET['pesan'])) {
                if ($_GET['pesan'] == 'berhasil') {
                    echo "<div class='alert alert-success'>Berhasil Mengganti Nama CoffeeShop</div>";
			...
			<input type="hidden" name="id" value="<?php echo $result[0]['id'] ?>">
						<div class="form-group">
							<label for="exampleInputEmail1">Nama Coffeeshop</label>
							<div class="input-group">
								<input type="text" class="form-control" placeholder="nama" name="nama" value="<?php echo $result[0]['nama'] ?>">
							</div>
						</div>
						
						<div class=" row d-flex justify-content-center align-content-center "><button type="submit" name="submit" class="btn btn-primary">Edit</button></div>
					</form>
				</div>
			</div>
		</div>
